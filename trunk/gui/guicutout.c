...
int EnableCutout(HWND hwndFrame, HBITMAP hbmSkin) {
	...
	HRESULT hasDWM = LoadLibrary("dwmapi.dll");
	if (hasDWM) {
		BOOL disableTransition = TRUE;
		DwmSetAttrib SetAttrib = GetProcAddress(hasDWM, "DwmSetWindowAttribute");
	...
	SetThemeFunc SetTheme = GetProcAddress(LoadLibrary("UxTheme.dll"), "SetWindowTheme");
	SetTheme(calcs[gslot].hwndLCD, TEXT (" "), TEXT (" "));
