    ...
    switch (Message) {
        ...
        case WM_NOTIFY:
		{
			...
			switch (((NMHDR*) lParam)->code) {
				case NM_CLICK: {
					HWND hVarName = GetDlgItem(hwnd, IDC_VAR_NAME);
					HWND hVarAdrr = GetDlgItem(hwnd, IDC_VAR_ADDRESS);
					HWND hVarRam = GetDlgItem(hwnd, IDC_VAR_RAM);
					HWND hVarPage = GetDlgItem(hwnd, IDC_VAR_PAGE);
					char text[256];
					nmtv->hdr;
					SendMessage(hVarName, WM_SETTEXT, 0, &text);
				}
				case NM_DBLCLK:	{
					int slot,i;
			...
			for(i=0;i<Tree[slot].sym.last - Tree[slot].sym.symbols + 1;i++) {
				if (Tree[slot].hVars[i]==hSelect) {
					if (Symbol_Name_to_String(&Tree[slot].sym.symbols[i],string)) {
						strcat(string,".");
						strcat(string,type_ext[Tree[slot].sym.symbols[i].type_ID]);
