...
int gui_frame_update(int slot) {
	...
	HBITMAP hbmOld = SelectObject(calcs[gslot].hdcSkin, hbmSkin);
	GetObject(hbmSkin, sizeof(BITMAP), &skinSize);
	char* name = malloc(strlen(CalcModelTxt[calcs[slot].model]) + 7);
	strcpy(name, CalcModelTxt[calcs[slot].model]);
	strcat(name, "Keymap");
	HBITMAP hbmKeymap = LoadBitmap(g_hInst, name);
	free(name);
	HBITMAP hbmOldKeymap = SelectObject(calcs[slot].hdcKeymap, hbmKeymap);
	GetObject(hbmKeymap, sizeof(BITMAP), &keymapSize);
	//skin and keymap must match
	...
	if ((skinSize.bmWidth != keymapSize.bmWidth) || (skinSize.bmHeight != keymapSize.bmHeight)) {
		...
		calcs[slot].rectSkin.bottom = skinSize.bmHeight;
	...
	if (hmenu != NULL) {
		if (!calcs[gslot].SkinEnabled) {
			...
			calcs[slot].hwndStatusBar = CreateWindowEx(0, STATUSCLASSNAME, NULL, WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, calcs[slot].hwndFrame, 99, g_hInst, NULL);
			SendMessage(calcs[slot].hwndStatusBar, SB_SETPARTS, 2, &iStatusWidths);
		...
		if (!calcs[slot].SkinEnabled || !calcs[slot].bCutout)
			FillRect(calcs[slot].hdcSkin, &rc, GetStockObject(GRAY_BRUSH));
		...
		if (!calcs[slot].SkinEnabled)
		{
			//HACK: probably not the best solution for this :|
...
#define GIFGRAD_TROUGH 10
			...
			/*if (calcs[gslot].SkinEnabled == TRUE && calcs[gslot].bCutout) {
