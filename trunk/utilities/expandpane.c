...
LRESULT CALLBACK ExpandPaneProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	switch (Message) {
		case WM_CREATE:
		{
			...
			SetWindowLongPtr(eps->hwndBtn, GWL_WNDPROC, (LONG_PTR) ExpandButtonProc);
			SetWindowLongPtr(eps->hwndBtn, GWL_USERDATA, (LONG_PTR) eps);
			SetWindowLongPtr(eps->hwndHeader, GWL_USERDATA, (LONG_PTR) eps);
...
INT_PTR QueryDebugKey(char *name) {
	...
	LONG rqvx_res = RegQueryValueEx(hkeyDebugger, name, NULL, NULL, &dwResult, &len);
...
void SaveDebugKey(char *name, DWORD *value) {
	...
	int error = RegSetValueEx(hkeyDebugger, name, 0, REG_DWORD, &value, sizeof(REG_DWORD));
