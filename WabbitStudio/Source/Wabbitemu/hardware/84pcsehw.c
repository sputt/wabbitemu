...
void port5_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		...
		change_page(cpu->mem_c, 3, cpu->bus % cpu->mem_c->ram_pages, TRUE);
		cpu->output = FALSE;
	}
}
...
void port6_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		...
		if (ram) {
			change_page(cpu->mem_c, 1, cpu->bus % cpu->mem_c->ram_pages, ram);
		} else {
			change_page(cpu->mem_c, 1, cpu->bus % cpu->mem_c->flash_pages, ram);
		}
		cpu->output = FALSE;
	}
}
...
void port7_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		...
		BOOL ram = (cpu->bus >> 7) & 1;
		if (ram)
			change_page(cpu->mem_c, 2, cpu->bus % cpu->mem_c->ram_pages, ram);
		else
			change_page(cpu->mem_c, 2, cpu->bus % cpu->mem_c->flash_pages, ram);
		cpu->output = FALSE;
	}
}
