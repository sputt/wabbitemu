...
static double timer_freq84pcse[4] = { 1.0 / 512.0, 1.0 / 227.0, 1.0 / 156.0, 1.0 / 108.0 };

void port0_84pcse(CPU_t *cpu, device_t *dev) {
...
// bit 6 - not used
// bit 7 - SE or Basic
void port2_84pcse(CPU_t *cpu, device_t *dev) {
...
void port3_84pcse(CPU_t *cpu, device_t *dev) {
...
void port4_84pcse(CPU_t *cpu, device_t *dev) {
	STDINT_t *stdint = (STDINT_t *) dev->aux;
...
void port5_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->bus = cpu->mem_c->banks[3].page;
		cpu->input = FALSE;
	} else if (cpu->output) {
		change_page(cpu, 3, cpu->bus % cpu->mem_c->ram_pages, TRUE);
		cpu->output = FALSE;
	}
}

void port6_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->bus = ((cpu->mem_c->banks[1].ram) << 7) + cpu->mem_c->banks[1].page;
		cpu->input = FALSE;
	} else if (cpu->output) {
		BOOL lastFlashPages = (~cpu->port0E) & 1;
		BOOL ram = (cpu->bus >> 7) & lastFlashPages;
		if (ram) {
			change_page(cpu, 1, cpu->bus % cpu->mem_c->ram_pages, ram);
		} else {
			change_page(cpu, 1, cpu->bus % cpu->mem_c->flash_pages, ram);
		}
		cpu->output = FALSE;
	}
}

void port7_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->bus = ((cpu->mem_c->banks[2].ram) << 7) + cpu->mem_c->banks[2].page;
		cpu->input = FALSE;
	} else if (cpu->output) {
		BOOL lastFlashPages = (~cpu->port0F) & 1;
		BOOL ram = (cpu->bus >> 7) & 1;
		if (ram) {
			change_page(cpu, 2, cpu->bus % cpu->mem_c->ram_pages, ram);
		} else {
			change_page(cpu, 2, cpu->bus % cpu->mem_c->flash_pages, ram);
		}
		cpu->output = FALSE;
	}
}
...
void port14_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
...
void port15_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->bus = 0x33;
...
void port8_84pcse(CPU_t *cpu, device_t *dev) {
...
void port9_84pcse(CPU_t *cpu, device_t *dev) {
	LINKASSIST_t* assist = (LINKASSIST_t *) dev->aux;
...
void port0A_84pcse(CPU_t *cpu, device_t *dev) {
	LINKASSIST_t *assist = (LINKASSIST_t *) dev->aux;
	if (cpu->input) {
		cpu->bus = assist->in;
		assist->read = FALSE;
...
void port0D_84pcse(CPU_t *cpu, device_t *dev) {
	LINKASSIST_t *assist = (LINKASSIST_t *) dev->aux;
	if (cpu->input) {
		cpu->bus = assist->out;
		assist->ready = FALSE;
...
void port0E_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->input = FALSE;
		cpu->bus = cpu->port0E;
	} else if (cpu->output) {
		cpu->output = FALSE;
		cpu->port0E = cpu->bus & 0x3;
		if (!cpu->mem_c->normal_banks[1].ram) {
			change_page(cpu, 1, cpu->mem_c->normal_banks[1].page, FALSE);
			if (cpu->mem_c->boot_mapped) {
				change_page(cpu, 2, cpu->mem_c->normal_banks[2].page, FALSE);
			}
		}
	}
}

void port0F_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->input = FALSE;
		cpu->bus = cpu->port0F;
	} else if (cpu->output) {
		cpu->output = FALSE;
		cpu->port0F = cpu->bus & 0x3;
		if (!cpu->mem_c->normal_banks[2].ram) {
			change_page(cpu, 2, cpu->mem_c->normal_banks[2].page, FALSE);
			if (cpu->mem_c->boot_mapped) {
				change_page(cpu, 1, cpu->mem_c->normal_banks[1].page, FALSE);
			}
...
void port20_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
...
void port21_84pcse(CPU_t *cpu, device_t *dev) {
...
void port22_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
...
void port23_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
...
void port24_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
		cpu->input = FALSE;
...
void port25_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
...
void port26_84pcse(CPU_t *cpu, device_t *dev) {
	if (cpu->input) {
...
void port_chunk_remap_84pcse(CPU_t *cpu, device_t *dev) {
...
void port30_84pcse(CPU_t *cpu, device_t *dev) {
...
void port31_84pcse(CPU_t *cpu, device_t *dev) {
	XTAL_t* xtal = (XTAL_t *) dev->aux;
	TIMER_t* timer = &xtal->timers[(DEV_INDEX(dev) - 0x30) / 3];
	if (cpu->input) {
...
void port32_84pcse(CPU_t *cpu, device_t *dev) {
	XTAL_t* xtal = (XTAL_t *) dev->aux;
	TIMER_t* timer = &xtal->timers[(DEV_INDEX(dev)-0x30)/3];
...
void port4A_84pcse(CPU_t *cpu, device_t *dev) {
...
void port4C_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
...
void port4D_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
...
void port54_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
...
void port55_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux; 
	if (cpu->input) {
...
void port56_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
...
void port57_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
...
void port5B_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
		cpu->bus = 0x00;
...
void port80_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
...
void port_84pcse(CPU_t *cpu, device_t *dev) {
	USB_t *usb = (USB_t *) dev->aux;
	if (cpu->input) {
		cpu->input = FALSE;
	} else if (cpu->output) {
...
STDINT_t* INT84pcse_init(CPU_t* cpu) {
	...
	stdint->freq[0] = timer_freq84pcse[0];
	stdint->freq[1] = timer_freq84pcse[1];
	stdint->freq[2] = timer_freq84pcse[2];
	stdint->freq[3] = timer_freq84pcse[3];
...
link_t* link84pcse_init(CPU_t* cpu) {
...
int device_init_84pcse(CPU_t *cpu) {
	ClearDevices(cpu);

/* link */
	link_t * link = link84pcse_init(cpu);
	cpu->pio.devices[0x00].active = TRUE;
	cpu->pio.devices[0x00].aux = link;
	cpu->pio.devices[0x00].code = (devp) port0_84pcse;
...
/* status */
	...
	cpu->pio.devices[0x02].code = (devp) port2_84pcse;

/* standard interrupts */
	STDINT_t* stdint = INT84pcse_init(cpu);
	cpu->pio.devices[0x03].active = TRUE;
	cpu->pio.devices[0x03].aux = stdint;
	cpu->pio.devices[0x03].code = (devp) port3_84pcse;

	cpu->pio.devices[0x04].active = TRUE;
	cpu->pio.devices[0x04].aux = stdint;
	cpu->pio.devices[0x04].code = (devp) port4_84pcse;
	
	
/* memory mapping */
	cpu->pio.devices[0x05].active = TRUE;
	cpu->pio.devices[0x05].code = (devp) port5_84pcse;

	cpu->pio.devices[0x06].active = TRUE;
	cpu->pio.devices[0x06].code = (devp) port6_84pcse;

	cpu->pio.devices[0x07].active = TRUE;
	cpu->pio.devices[0x07].code = (devp) port7_84pcse;
...
/* LCD */
	C_LCD_t *lcd = CLCD_init(cpu);
	cpu->pio.devices[0x10].active = TRUE;
	cpu->pio.devices[0x10].aux = lcd;
	cpu->pio.devices[0x10].code = (devp) CLCD_command;
	cpu->pio.devices[0x11].active = TRUE;
	cpu->pio.devices[0x11].aux = lcd;
	cpu->pio.devices[0x11].code = (devp) CLCD_data;

/* Flash locking */
	cpu->pio.devices[0x14].active = TRUE;
	cpu->pio.devices[0x14].code = (devp) port14_84pcse;
	...
	cpu->pio.devices[0x15].code = (devp) port15_84pcse;
...
// weird ass remap stuff
	...
	cpu->pio.devices[0x27].code = (devp) &port_chunk_remap_84pcse;

	cpu->pio.devices[0x28].active = TRUE;
	cpu->pio.devices[0x28].aux = &cpu->mem_c->port28_remap_count;
	cpu->pio.devices[0x28].code = (devp) &port_chunk_remap_84pcse;
