...
namespace Revsoft.Wabbitcode
{
    public class Assembler
    {
        ...
        public Assembler()
        {
            ...
            assembler.DoWork += assembleProject;
            assembler.RunWorkerCompleted += doneAssembling;
        ...
        public Assembler(string text, ASM_MODE CodeCount)
        {
            ...
            switch (CodeCount)
            {
                ...
                case ASM_MODE.MODE_COMMANDLINE:
                    ...
                    filePath = text;
                    assembler = new BackgroundWorker();
                    assembler.DoWork += runCommandLine;
                    assembler.RunWorkerAsync();
                    break;
                case ASM_MODE.MODE_NORMAL:
                    ...
                    GlobalClass.mainForm.pipeServer.MessageReceived += GlobalClass.mainForm.pipeServer_MessageReceived;
                    GlobalClass.mainForm.pipeServer.MessageReceived -= GlobalClass.mainForm.pipeServer_HandleCodeCount;
                    assembler.DoWork += assembleCode;
                    assembler.RunWorkerCompleted += doneAssembling;
                    filePath = text;
                    assembledName = Path.ChangeExtension(text, getExtension(Settings.Default.outputFile));
                    assembler.RunWorkerAsync();
                    GlobalClass.outputWindow.outputWindowBox.Text = "==================" + Path.GetFileName(filePath) + "==================\r\n" +
                                                     "Assembling " + Path.GetDirectoryName(filePath) + "\\" + Path.GetFileName(filePath) + "\r\n";
                    break;
            }
        }

        void doneAssembling(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            GlobalClass.errorList.parseOutput(outputText, Path.GetDirectoryName(filePath));
            if (errors)
        ...
        private void countCode(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                RunAssemblyWithArguments(filePath, IntPtr.Zero, IntPtr.Zero);
            }
            catch (Exception)
            { }
        }
        ...
        private void assembleCode(object sender, DoWorkEventArgs e)
        {
            ...
            string fileName = Path.GetFileName(filePath);//.Substring(filePath.LastIndexOf('\\') + 1, filePath.Length - filePath.LastIndexOf('\\') - 1);
            //now we can set the args for spasm
            int error = 0;
