...
namespace Revsoft.Wabbitcode.Docking_Windows
{
    public partial class ProjectViewer : ToolWindow
    {
        ...
        public void BuildProjTree()
        {
            ...
            this.RecurseAddNodes(folder, null);
        ...
        {
            this.DeleteNodes(projViewer.SelectedNodes);
        }
        ...
        {
            this.OpenNode(projViewer.SelectedNode);
            foreach (TreeNode node in projViewer.SelectedNodes)
            {
                this.OpenNode(node);
            }
        ...
        private void OpenNode(TreeNode dropNode)
        {
            ...
            ProjectFile file = this.GetFileFromPath(dropNode.FullPath);
            if (file == null)
            {
                return;
            }
        ...
        {
            TreeNode nodeAdded = this.AddFolder(folder, parentNode);
            foreach (ProjectFolder subFolder in folder.Folders)
            {
                this.RecurseAddNodes(subFolder, nodeAdded);
            }
            foreach (ProjectFile file in folder.Files)
            {
                this.AddFile(file, nodeAdded);
            }
        }
        ...
        {
            this.RenameFolder(projViewer.SelectedNode);
        }

        private void renMenuItem_Click(object sender, EventArgs e)
        {
            this.RenameFile(projViewer.SelectedNode);
        }
    }
