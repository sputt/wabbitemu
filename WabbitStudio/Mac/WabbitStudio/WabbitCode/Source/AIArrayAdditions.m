...
- (void)moveObjects:(NSArray *)objects toIndexes:(NSIndexSet *)newIndexes; {
	NSUInteger index, count = [newIndexes count];
	NSUInteger indexes[count];
	
	[newIndexes getIndexes:indexes maxCount:count inIndexRange:NULL];
	
	for (index = 0; index < count; index++) {
		[self moveObject:[objects objectAtIndex:index] toIndex:indexes[index]];
	}
}
