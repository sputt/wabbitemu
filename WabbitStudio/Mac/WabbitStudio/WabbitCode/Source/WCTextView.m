...
- (void)drawViewBackgroundInRect:(NSRect)rect {
	...
	if ([[NSUserDefaults standardUserDefaults] boolForKey:kWCPreferencesEditorErrorLineHighlightKey]) {
	...
	if ([[NSUserDefaults standardUserDefaults] boolForKey:kWCPreferencesEditorWarningLineHighlightKey]) {
...
- (NSArray *)userDefaultsKeys {
	return [NSArray arrayWithObjects:kWCPreferencesEditorTextColorKey,kWCPreferencesEditorBackgroundColorKey,kWCPreferencesEditorErrorLineHighlightKey,kWCPreferencesEditorErrorLineHighlightColorKey,kWCPreferencesEditorWarningLineHighlightKey,kWCPreferencesEditorWarningLineHighlightColorKey, nil];
}
...
- (IBAction)jumpToDefinition:(id)sender; {
	...
	if (![symbols count]) {
		...
		for (WCSymbol *symbol in symbols) {
			NSMenuItem *item = [menu addItemWithTitle:[NSString stringWithFormat:@"%@ - %@",[symbol name],[[symbol file] name]] action:@selector(_jumpToSymbolFromMenu:) keyEquivalent:@""];
