...
- (void)performSyntaxHighlighting; {
	...
	NSSet *labelStrings = nil;
	NSSet *equateStrings = nil;
	NSSet *defineStrings = nil;
	NSSet *macroStrings = nil;
	NSArray *equateSymbols = nil;
	
	if ([[_textView file] project]) {
		labelStrings = [[[_textView file] project] labelStrings];
		equateStrings = [[[_textView file] project] equateStrings];
		defineStrings = [[[_textView file] project] defineStrings];
		macroStrings = [[[_textView file] project] macroStrings];
		equateSymbols = [[[_textView file] project] equateSymbols];
	}
	else {
		labelStrings = [[[_textView file] symbolScanner] labelStrings];
		equateStrings = [[[_textView file] symbolScanner] equateStrings];
		defineStrings = [[[_textView file] symbolScanner] defineStrings];
		macroStrings = [[[_textView file] symbolScanner] macroStrings];
		equateSymbols = [[[_textView file] symbolScanner] equateSymbols];
	}
	...
	BOOL symbolsAreCaseSensitive = [[[[_textView file] project] activeBuildTarget] labelsAreCaseSensitive];
	...
	while ([symbolsEnum nextRanges] != NULL) {
		...
		if (highlightEquates && [equateStrings containsObject:symbolName]) {
			...
			if (showEquateValues) {
				for (WCSymbol *symbol in equateSymbols) {
					NSString *ssymbolName = (symbolsAreCaseSensitive)?[symbol name]:[[symbol name] lowercaseString];
					if ([ssymbolName isEqualToString:symbolName]) {
						[[_textView layoutManager] addTemporaryAttribute:NSToolTipAttributeName value:[symbol symbolValue] forCharacterRange:actualRange];
						break;
					}
				}
			}
		}
		else if (highlightLabels && [labelStrings containsObject:symbolName])
		...
		else if (highlightDefines && [defineStrings containsObject:symbolName])
		...
		else if (highlightMacros && [macroStrings containsObject:symbolName])
