...
static NSString *kWCPreferencesLastSelectedIdentifierKey = @"kWCPreferencesLastSelectedIdentifierKey";
...
- (IBAction)showWindow:(id)sender 
{
	...
	NSString *firstIdentifier = [[NSUserDefaults standardUserDefaults] objectForKey:kWCPreferencesLastSelectedIdentifierKey];
	if (!firstIdentifier || ![[[[[self window] toolbar] items] valueForKey:@"itemIdentifier"] containsObject:firstIdentifier])
...
{	
	...
	if (![newView isEqualTo:oldView]) {		
		...
		[[NSUserDefaults standardUserDefaults] setObject:identifier forKey:kWCPreferencesLastSelectedIdentifierKey];
	}
}
...
- (void)crossFadeView:(NSView *)oldView withView:(NSView *)newView
{
	...
	[newView setHidden:NO]; // fix for the NSViewAnimation fade in effect not working on 10.6
