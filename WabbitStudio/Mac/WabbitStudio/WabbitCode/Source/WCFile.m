...
NSString* const kWCFileURLKey = @"URL";
NSString* const kWCFileBookmarkDataKey = @"bookmarkData";
...
- (BOOL)saveFile:(NSError **)outError; {
	...
	if (![string writeToURL:[self URL] atomically:YES encoding:NSUTF8StringEncoding error:outError])
		return NO;
...
- (void)setName:(NSString *)name {
	...
	// rename by moving
	if (![[NSFileManager defaultManager] moveItemAtPath:[self filePath] toPath:renamePath error:NULL])
		return;
...
- (void)_setupTextStorageAndSymbolScanner; {
	if (!_textStorage) {
		NSString *string = [[[NSString alloc] initWithContentsOfURL:[self URL] encoding:NSUTF8StringEncoding error:NULL] autorelease];
