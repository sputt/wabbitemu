...
void and_reg8(CPU_t *cpu) {
	...
	switch ((cpu->bus)&7) {
		...
		case 0x04:
			index_ext (
				reg = cpu->h;,
				reg = cpu->ixh; tc_sub(cpu->timer_c,4);,
				reg = cpu->iyh; tc_sub(cpu->timer_c,4);
			)
			break;
		case 0x05:
			index_ext (
				reg = cpu->l;,
				reg = cpu->ixl; tc_sub(cpu->timer_c, 4);,
				reg = cpu->iyl; tc_sub(cpu->timer_c, 4);
			)
			break;
		case 0x06:
			if (!cpu->prefix) {
