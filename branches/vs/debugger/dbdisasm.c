...
LRESULT CALLBACK DisasmProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	switch (Message) {
		...
		case WM_CONTEXTMENU: {
			...
			HMENU hmenu = LoadMenu(g_hInst, MAKEINTRESOURCE(IDR_DISASM_MENU));
			if (!hmenu) break;
		...
		case WM_PAINT: {
			...
			int lpiArray[8];
			int iSize = SendMessage(dps->hwndHeader, HDM_GETITEMCOUNT, 0, 0);
			if (iSize == 0) return 0;
			...
			int end_i = (ps.rcPaint.bottom - cyHeader + dps->cyRow - 1)/dps->cyRow;
		...
		case WM_LBUTTONDOWN: {
			...
			int y = GET_Y_LPARAM(lParam) - (hdrRect.bottom - hdrRect.top);
			RECT r;
		...
		case WM_LBUTTONUP: {
			RECT hdrRect;
			GetWindowRect(dps->hwndHeader, &hdrRect);
			int y = GET_Y_LPARAM(lParam) - cyHeader;
		...
		case WM_KEYDOWN: {
			...
			switch (wParam) {
				...
				case VK_PRIOR:
					...
					break;
				case 'G': {
					SendMessage(hwnd, WM_COMMAND, DB_DISASM_GOTO, 0);
					break;
				}
			}
