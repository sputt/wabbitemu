...
int CreateToolbarButton(HWND hwndParent, char *szCaption, char *szTooltip, char *szIcon, int x, int y, int ID) {
	...
	if (strlen(szCaption) == 0)
		img_sz_seperator = 0;
	MoveWindow(hwndBtn, x, y, 4 + 16 + img_sz_seperator + r.right + 4, 24, FALSE);
		...
		case WM_CREATE:
			{
				...
				int next = 4;
				next = CreateToolbarButton(hwnd, "Run", "Run the calculator.", "DBRun", next, 4, 1001);
				next = CreateToolbarButton(hwnd, "Toggle Breakpoint", "Toggle the breakpoint on the current selection.", "DBBreak", next, 4, 1002);
				next = CreateToolbarButton(hwnd, "Step", "Run a single command.", "DBStep", next, 4, 999);
				next = CreateToolbarButton(hwnd, "Step Over", "Run a single line.", "DBStepOver", next, 4, 1000);
				next = CreateToolbarButton(hwnd, "Goto", "Goto an address in RAM or Flash.", "DBGoto", next, 4, 1003);
				next = CreateToolbarButton(hwnd, "Track", "Create a variable track window.", "DBGoto", next, 4, 1005);
				next = CreateToolbarButton(hwnd, "", "Display additional commands.", "Chevron", next, 4, 1004);
	...
	if (label) { \
		...
		AddDebugTrack(gslot, zz, waddr, ss, ff); \
				...
				case 1004:
				{
					...
					void **ppvBits = NULL;
					CreateDIBSection(GetDC(hwnd), &biMenu, DIB_RGB_COLORS, ppvBits, NULL, 0); 
