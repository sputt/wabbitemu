...
int gui_frame(int slot) {
	...
	GetWindowRect(calcs[slot].hwndFrame, &r);
...
int gui_frame_update(int slot) {
	HDC hdc = GetDC(calcs[slot].hwndFrame);
	TCHAR szPath[MAX_PATH] = "Skins\\";
	
	_tcscat(szPath, CalcModelTxt[calcs[slot].model]);
	_tcscat(szPath, ".png");

	HBITMAP CreateBitmapFromPNG(const TCHAR *szFilename);

	//HBITMAP hbmSkin = CreateBitmapFromPNG(szPath);
	//HBITMAP hbmOld = (HBITMAP) SelectObject(calcs[gslot].hdcSkin, hbmSkin);
	//DeleteObject(hbmOld);
	
	//HBITMAP hbmKeymap = CreateBitmapFromPNG("Skins\\TI-83+Keys.png");
	//hbmOld = (HBITMAP) SelectObject(calcs[gslot].hdcKeymap, hbmKeymap);
	//DeleteObject(hbmOld);
...
#define GIFGRAD_TROUGH 10
			...
			if (calcs[gslot].SkinEnabled == TRUE && calcs[gslot].bCutout && GetForegroundWindow() != hwnd) {
				...
				FillRect(hdcOverlay, &rc, GetStockBrush(GRAY_BRUSH));
				BLENDFUNCTION bf;
				bf.BlendOp = AC_SRC_OVER;
				bf.BlendFlags = 0;
			...
			switch (LOWORD(wParam)) {
				...
				case IDM_CALC_SKIN:
				{
					...
					if (calcs[gslot].SkinEnabled) {
						...
						// Create status bar
						RECT rc;
						SetRect(&rc, 0, 0, 128*calcs[gslot].Scale, 64*calcs[gslot].Scale);
						
						calcs[gslot].SkinEnabled = FALSE;
						
						int iStatusWidths[] = {100, -1};
						calcs[gslot].hwndStatusBar = CreateWindowEx(0, STATUSCLASSNAME, NULL, WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, hwnd, (HMENU) 99, g_hInst, NULL);
						SendMessage(calcs[gslot].hwndStatusBar, SB_SETPARTS, 2, (LPARAM) &iStatusWidths);
		...
		case WM_KEYDOWN:
			{
				...
				if (kp) {
					...
					if ((calcs[gslot].cpu.pio.keypad->keys[kp->group][kp->bit] & KEY_STATEDOWN) == 0) {
						DrawButtonState(calcs[gslot].hdcSkin, calcs[gslot].hdcKeymap, &ButtonCenter[kp->bit+(kp->group<<3)], DBS_DOWN | DBS_PRESS);
		...
		case WM_NCHITTEST:
		{
			...
			if (GetRValue(GetPixel(calcs[gslot].hdcKeymap, pt.x, pt.y)) != 0xFF) return htRet;
			return HTCAPTION;
		}
		default:
